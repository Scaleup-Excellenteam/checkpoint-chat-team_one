...
export default function HomePage() {
  ...
  const handleCreateRoom = async (name: string) => {
    ...
    if (!trimmed) return;
    try {
      const created = await RoomsAPI.create(trimmed);
      setRooms((prev) =>
    ...
    try {
      ...
      if (typeof roomOrName === "string") {
        ...
        if (!room) {
          ...
          setRooms(fresh);
          room = fresh.find((r) => r.name === targetName);
        }
      } else {
        room = roomOrName;
      }
      ...
      // Ask WS server to join room by id (and pass name as fallback)
      sendWebSocketMessage({ type: "join_room", roomId: room._id, room: room.name });
    } catch (e: any) {
